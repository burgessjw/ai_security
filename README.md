...
|✅| 自定义openclaw镜像|替换基础镜像中的node版本为24+；安装openclaw<br>https://hub.docker.com/repository/docker/jarv1swho/opensandbox/general|
|✅|运行openclaw|脚本创建沙盒；首次创建成功后，在沙盒内执行openclaw onboard进行配置provider，挂载.openclaw目录，实现重复使用|
|✅|调用openclaw|脚本调用,写爬虫爬虎扑<br>|
|✅|安装skill，文章去ai化|https://skillhub.cn/skills/user_ab5ae6ee/unclecheng-reduce-ai-perception-v2|
|✅|接入微信|openclaw plugins install "@tencent-weixin/openclaw-weixin" --force<br>微信发送指令写爬虫爬虎扑；微信发送指令给文章去ai化|
